          ...
          <div className="space-y-5">
            ...
            {((candidate.databases?.length ?? 0) > 0 || (candidate.frameworks?.length ?? 0) > 0 || (candidate.tools?.length ?? 0) > 0) && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                {(candidate.frameworks?.length ?? 0) > 0 && <div className="bg-white rounded-2xl p-5 border border-gray-100"><h3 className="font-bold text-gray-700 text-sm mb-3">⚙️ Frameworks</h3><div className="flex flex-wrap gap-2">{candidate.frameworks!.map(f => <span key={f} className="bg-violet-50 text-violet-700 text-xs font-medium px-2.5 py-1 rounded-full border border-violet-100">{f}</span>)}</div></div>}
                {(candidate.tools?.length ?? 0) > 0 && <div className="bg-white rounded-2xl p-5 border border-gray-100"><h3 className="font-bold text-gray-700 text-sm mb-3">🛠️ Tools</h3><div className="flex flex-wrap gap-2">{candidate.tools!.map(t => <span key={t} className="bg-gray-100 text-gray-700 text-xs font-medium px-2.5 py-1 rounded-full">{t}</span>)}</div></div>}
              </div>
            )}
          </div>
        )}
          ...
          <div className="space-y-5">
            <div className="bg-white rounded-2xl p-6 border border-gray-100">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="h-4 bg-gray-100 rounded-full"><div className={`h-4 rounded-full ${score >= 80 ? "bg-emerald-500" : score >= 60 ? "bg-blue-500" : "bg-amber-500"}`} style={{ width: `${score}%` }} /></div>
              ...
              <div className="bg-white rounded-2xl p-6 border border-gray-100">
                ...
                <div className="space-y-4">
                  {candidate.skillScores!.map(({ skill, score: s }) => (
                    <div key={skill}>
                      <div className="flex justify-between text-sm mb-1.5"><span className="font-medium text-gray-700">{skill}</span><span className="font-bold text-gray-900">{s}/100</span></div>
                      <div className="h-2.5 bg-gray-100 rounded-full"><div className={`h-2.5 rounded-full ${s >= 80 ? "bg-emerald-500" : s >= 60 ? "bg-blue-500" : "bg-amber-500"}`} style={{ width: `${s}%` }} /></div>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {candidate.skillScores!.map(({ skill, score: s }) => (
                    <div key={skill} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 h-2 bg-gray-100 rounded-full"><div className={`h-2 rounded-full ${s >= 80 ? "bg-emerald-500" : s >= 60 ? "bg-blue-500" : "bg-amber-500"}`} style={{ width: `${s}%` }} /></div>
