...
export default function JobsPage() {
  ...
  async function createJob() {
    ...
    try {
      ...
      if (res.ok) { setShowModal(false); setNewJob({ title: "", department: "", location: "", description: "" }); fetchJobs(); }
          ...
          {filtered.map(job => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full capitalize ml-2 shrink-0 ${statusColors[job.status]}`}>
                  {job.status}
                </span>
              </div>
              <div className="space-y-1.5 text-sm text-gray-500">
                ...
                <div className="flex items-center gap-2">📅 <span>{new Date(job.createdAt).toLocaleDateString()}</span></div>
              </div>
          ...
          <div className="bg-white rounded-2xl p-8 w-full max-w-lg shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea value={newJob.description} onChange={e => setNewJob({ ...newJob, description: e.target.value })}
                  rows={4} placeholder="Paste your job description here..."
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowModal(false)}
                className="flex-1 border border-gray-200 py-2.5 rounded-xl font-semibold text-gray-600 hover:bg-gray-50 transition-all">
                Cancel
              </button>
